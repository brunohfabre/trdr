...
while True:
  min = float(((datetime.now()).strftime('%M.%S')))

  if (min >= 59.58 and min <= 0) or (min >= 14.58 and min <= 15) or (min >= 29.58 and min <= 30) or (min >= 44.58 and min <= 45):
    candles = Iq.get_candles(pair, 60 * 5, 3, time.time())
