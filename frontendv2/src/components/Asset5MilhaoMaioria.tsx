...
export function Asset5MilhaoMaioria({ asset }: AssetProps): JSX.Element {
  ...
  useEffect(() => {
    const result = asset.candles.filter((candle, index) => {
      ...
      if (
        analysisCandles.filter(item => item.color === 'green').length === analysisCandles.filter(item => item.color === 'red').length
      ) {
        entry = 'doji';
      }
      ...
      if (
        analysisCandles.filter(item => item.color === 'red').length > analysisCandles.filter(item => item.color === 'green').length
      ) {
        entry = 'red';
      }
