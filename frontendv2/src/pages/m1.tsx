...
import {
  IStrategyProcessed,
  processAssetsM1,
} from '../helpers/processAssetsM1';

type IStrategy = 'mhimin' | 'mhimin';

export default function M1(): JSX.Element {
  ...
  const [selectedStrategy, setSelectedStrategy] = useState<IStrategy>('mhimin');
  ...
  useEffect(() => {
    if (assets.length) {
      const response = processAssetsM1(assets);
